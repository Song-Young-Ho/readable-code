...
public class MinesweeperGame {
    ...
    private static Integer[][] NEARBY_LANE_MINE_COUNTS = new Integer[BOARD_ROW_SIZE][BOARD_COL_SIZE];
    ...
    private static void checkIfGameIsOver() {
        if (isAllCellOpened()) changeGameStatusToWin();
    }
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (isLandMineCell(row, col)) {
                    NEARBY_LANE_MINE_COUNTS[row][col] = 0;
                    continue;
                }
                int count = countNearbyLandMines(row, col);
                NEARBY_LANE_MINE_COUNTS[row][col] = count;
            }
        }
    }
    ...
    private static void open(int row, int col) {
        ...
        if (NEARBY_LANE_MINE_COUNTS[row][col] != 0) {
            BOARD[row][col] = String.valueOf(NEARBY_LANE_MINE_COUNTS[row][col]);
            return;
        } else {
